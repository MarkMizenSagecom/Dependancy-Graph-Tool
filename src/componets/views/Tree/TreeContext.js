...
export function TreeContextProvider({ children }) {
  ...
  const [nodes, maxColumns] = useMemo(() => {
    ...
    return [nodesAndPos, columnsLength];
  }, [columns, connections, itemId]);

  // console.log({ connections, columns, nodesAndPos });
  const active = useMemo(() => {
    return nodes.findIndex((node) => node.id === itemId);
  }, [nodes, itemId]);

  return (
    <TreeContext.Provider value={{ nodes, maxColumns, active }}>
      {children}
